...
class MusicLibraryController
    ...
    def list_songs_by_genre
        ...
        input = gets.strip
        all_genre = Song.all.select {|song| song.genre.name == input}
        all_genre_sort = all_genre.sort {|a, b| a.name <=> b.name}
        all_genre_sort.each_with_index {|song, i| puts "#{i+1}. #{song.artist.name} - #{song.name}"}
    end

    def play_song
        puts "Which song number would you like to play?"
        input = gets.strip.to_i #remove decimal by setting to integer
        songs = Song.all.sort {|a, b| a.name <=> b.name}
